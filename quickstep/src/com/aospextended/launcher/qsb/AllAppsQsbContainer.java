 ...
 */
package com.aospextended.launcher.qsb;
...
import com.aospextended.launcher.qsb.configs.ConfigurationBuilder;
import com.aospextended.launcher.search.SearchHandler;
...
public class AllAppsQsbContainer extends FrameLayout implements Insettable, OnClickListener, OnChangeListener, OnUpdateListener, SearchUiManager {
    ...
    public CustomLauncher mLauncher;
    ...
    public AllAppsQsbContainer(Context context, AttributeSet attributeSet, int res) {
        ...
        mResult = 0;
        mAlpha = 0;
        mLauncher = (CustomLauncher) Launcher.getLauncher(context);
    ...
    public void onAttachedToWindow() {
        ...
        WallpaperColorInfo instance = WallpaperColorInfo.INSTANCE.get(getContext());
    ...
    public void onDetachedFromWindow() {
        ...
        WallpaperColorInfo.INSTANCE.get(getContext()).removeOnChangeListener(this);
    ...
    public void onClick(View view) {
        if (mLauncher.isInState(ALL_APPS)) {
            mLauncher.startSearch("", false, null, true);
        } else {
            mLauncher.getStateManager().goToState(ALL_APPS);
            new Handler().postDelayed(() -> mLauncher.startSearch("", false, null, true),
                    SEARCH_TASK_DELAY_MS);
        }
    }
    ...
    public void drawCanvas(Canvas canvas, int width) {
        ...
        if (mSearchIconStrokeWidth > SWIPE_DURATION_MULTIPLIER && mSearchIcon.getVisibility() == 0) {
    ...
    private Bitmap createBitmap(float shadowBlur, float keyShadowDistance, int color) {
        ...
        pill = pill.copy(Bitmap.Config.ARGB_8888, true);
    ...
    protected void updateQsbType() {
        boolean useDefaultQsb = !hasPackageInstalled(Launcher.getLauncher(mContext), CustomLauncher.SEARCH_PACKAGE);
    ...
    public boolean hasPackageInstalled(Context context, String pkgName) {
        try {
    ...
    public EditText setTextSearchEnabled(boolean isEnabled) {
        EditText editText = mLauncher.getAppsView().getSearchUiManager().setTextSearchEnabled(true);
        return editText;
    }
}
