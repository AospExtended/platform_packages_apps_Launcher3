...
public abstract class RecentsView<T extends StatefulActivity> extends PagedView implements
    ...
    public Set<String> sLockedTasks = new HashSet<>();
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        mLightLockedDrawable = context.getDrawable(R.drawable.recents_locked);
        mLightUnlockedDrawable = context.getDrawable(R.drawable.recents_unlocked);
    }
    ...
    protected void onPageEndTransition() {
        ...
        if (sLockedTasks.contains(mStartPkg) != sLockedTasks.contains(mEndPkg))
            updateLockTaskDrawable();
        if (getNextPage() > 0) {
    ...
    private void removeTasksViewsAndClearAllButton() {
        for (int i = getTaskViewCount() - 1; i >= 0; i--) {
            TaskView tv = getTaskViewAt(i);
            if (sLockedTasks.contains(tv.getTask().key.getPackageName())) continue;
    ...
    private void updateLockTaskDrawable(String pkg) {
        mLockButtonView.setCompoundDrawablesRelativeWithIntrinsicBounds(
                sLockedTasks.contains(pkg) ? mLightLockedDrawable :
                mLightUnlockedDrawable, null, null, null);
        mLockButtonView.setText(
                sLockedTasks.contains(pkg) ? "Locked" : "Unlocked");
    ...
    private void lockCurrentTask(View view) {
        ...
        if (taskView != null) {
            ...
            if (sLockedTasks.contains(pkg)) {
                sLockedTasks.remove(pkg);
            } else {
                sLockedTasks.add(pkg);
            }
            updateLockTaskDrawable(pkg);
        }
        Settings.Secure.putStringForUser(getContext().getContentResolver(), "recents_locked_tasks", sLockedTasks.toString(),
